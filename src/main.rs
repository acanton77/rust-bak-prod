...
fn main() {
    ...
    let mut message_data : String;
    //let mut html_dir_name = String::from("");
    let mut zip_in_file : String;
    let mut zip_out_file_name : String;
    let mut rsync_dir : String;
    let mut database: String;
    let mut log_path : String;
    let mut dump_out_file_name : String;
    ...
    for line in &vec_switch_file {
        ...
        if line == "espodb" {
           ...
           // &espo_vec.sort();
            ...
            let _ = &espo_vec.reverse();
            let count2 = espo_vec.len();

            for i in 4..count2 {
                // println!("deleted: {}", &espo_vec[i]);
                let file_name_to_be_deleted2 = &espo_vec[i];
                let _ = fs::remove_file(file_name_to_be_deleted2);
        ...
        */
        ...
        if line == "ac_addressbook" {
            ...
            .unwrap()
            {
                let orig_file_name: String = file.unwrap().path().display().to_string();
            ...
            for i in 4..count {
                let file_name_to_be_deleted = &my_vec[i];
                let _ = fs::remove_dir_all(file_name_to_be_deleted);
            }
 ...
 */
        ...
        if line == "anc123" {
            ...
            */
            ...
            let _ = fs::remove_file(anc123_zip_file);
        } // end anc123
          //@
        ...
        if line == "scripts" {
            ...
            let _ = fs::remove_file(rs_bak_prod_zip_file);
        } // end anc123
          //@
...
fn bak_bootstrap( zip_in_file: String,  zip_out_file_name: String,  rsync_dir: &String) {
    ...
    let _ = fs::remove_file(zip_out_file);
} //end bootstrap
  //@
...
fn bak_wordpress(
    ...
    let _custom_format22 = current_local22.format("%b %d %H:%M:%S");
    ...
    let  sql_in: String = dump_out_file.to_string();
    ...
    let _ = fs::remove_file(db_zip);
    let _ = fs::remove_file(sql_in);
} // end wordpress
  //@
...
fn send_mail(msg_vec: &mut Vec<(String, String, String)>, vec_switch_file: &Vec<String>) {
    ...
    let mut num = 0;
    let mut _newvalue: String;
    let mut _from_orig: String;
